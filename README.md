# 🚀 LeetCode Questions

This repository contains my solutions to various LeetCode problems. The goal is to improve problem-solving skills, strengthen understanding of Data Structures & Algorithms, and prepare for technical interviews.

---

## 📂 Folder Structure

The problems are organized based on difficulty:

- Easy  
- Medium  
- Hard  

Each folder contains solutions categorized accordingly.

---

## 🧠 Topics Covered

- Arrays  
- Strings  
- Linked List  
- Trees  
- Recursion  
- Dynamic Programming  
- Sorting & Searching  

---

## 💻 Language Used

- Python

---

## 🎯 Objectives

- Practice Data Structures & Algorithms  
- Build consistency in problem solving  
- Prepare for coding interviews  
- Strengthen logical thinking  

---

## 📌 Notes

- Each solution includes clean and readable code  
- Some problems include explanations and approaches  
- Solutions are regularly updated  

---

## 📈 Progress

- Total Problems Solved: (update this)  
- Easy: (update this)  
- Medium: (update this)  
- Hard: (update this)  

---

## ⭐ Support

If you find this repository helpful, consider giving it a star ⭐

---
